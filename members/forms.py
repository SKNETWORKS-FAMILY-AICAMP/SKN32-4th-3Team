...
class SignUpForm(UserCreationForm):
    ...
    같이 받아 가입과 동시에 Membership 신청까지 접수한다(members/views.py:
    SignUpView.post() 가 apartments.services.apply_for_membership() 을 호출).
    Member 모델에는 손대지 않는다 — 역할은 어디까지나 Membership 행으로만
    표현한다는 apartments 앱의 원칙을 그대로 지킨다.
    """
